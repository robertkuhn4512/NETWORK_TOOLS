...
# This avoids the common TLS SAN mismatch error when using https://vault_production_node:8200.
...
# TLS server name override for in-container Vault CLI. When we fall back to the container name,
# we still validate the server certificate against the preferred public hostname.
...
ensure_vault_cli_connectivity() {
  # Try preferred public hostname first (TLS-valid), then fall back to container DNS name.
  # When falling back, preserve TLS hostname verification via VAULT_TLS_SERVER_NAME_IN_CONTAINER.
  local tried=()

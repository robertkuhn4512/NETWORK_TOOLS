...
#
# Generates a local CA + a single Keycloak HTTPS server certificate/key for the Network Tools project.
#
# NOTES / HOW TO RUN
#   1) Place this script at:
#        $HOME/NETWORK_TOOLS/backend/build_scripts/generate_local_keycloak_certs.sh
#      Then:
#        chmod +x $HOME/NETWORK_TOOLS/backend/build_scripts/generate_local_keycloak_certs.sh
#
...
#        $HOME/NETWORK_TOOLS/backend/build_scripts/generate_local_keycloak_certs.sh \
#          --dns keycloak --dns keycloak.local --dns keycloak.yourdomain.edu \
#          --ip 127.0.0.1 --ip 172.16.99.150
#
#   6) Values file (optional, auto-loaded if present):
#        Default: $HOME/NETWORK_TOOLS/backend/build_scripts/keycloak_cert_values.env
#
#        Example contents:
#          KEYCLOAK_CERT_CN="keycloak"
#          KEYCLOAK_CERT_CA_CN="NETWORK_TOOLS Local Keycloak CA"
#          KEYCLOAK_CERT_CA_DAYS="3650"
#          KEYCLOAK_CERT_LEAF_DAYS="825"
#          KEYCLOAK_CERT_DNS_SANS="keycloak,localhost"
#          KEYCLOAK_CERT_IP_SANS="127.0.0.1"
#          KEYCLOAK_CERT_OUT_DIR="$PROJECT_ROOT_DEFAULT/backend/app/keycloak/certs"
#
# OUTPUTS (default):
#   $HOME/NETWORK_TOOLS/backend/app/keycloak/certs/
#     cert.key   (server key)
#     cert.crt   (fullchain: leaf + CA)
#     ca.crt     (CA cert)
#     ca.key     (CA key - protect this file)
#
# Intended usage (later, when you wire Keycloak HTTPS):
#   KC_HTTPS_CERTIFICATE_FILE=/run/vault/tls/server.crt   (or mount these directly)
#   KC_HTTPS_CERTIFICATE_KEY_FILE=/run/vault/tls/server.key
#
# For local-only testing, you can mount:
#   backend/app/keycloak/certs/cert.crt -> /opt/keycloak/conf/server.crt (or wherever you prefer)
#   backend/app/keycloak/certs/cert.key -> /opt/keycloak/conf/server.key
#
# and set:
...
OUT_DIR_DEFAULT="$PROJECT_ROOT_DEFAULT/backend/app/keycloak/certs"
VALUES_FILE_DEFAULT="$PROJECT_ROOT_DEFAULT/backend/build_scripts/keycloak_cert_values.env"

FORCE="0"
SKIP_VALUES_FILE="0"

CN_DEFAULT="keycloak"
CA_CN_DEFAULT="NETWORK_TOOLS Local Keycloak CA"
...
Examples:
  $0
  $0 --force
  $0 --dns keycloak --dns keycloak.local --ip 127.0.0.1
EOF
}
...
load_values_file() {
  ...
  OUT_DIR="${KEYCLOAK_CERT_OUT_DIR:-$OUT_DIR}"
  CN="${KEYCLOAK_CERT_CN:-$CN}"
  CA_CN="${KEYCLOAK_CERT_CA_CN:-$CA_CN}"
  CA_DAYS="${KEYCLOAK_CERT_CA_DAYS:-$CA_DAYS}"
  LEAF_DAYS="${KEYCLOAK_CERT_LEAF_DAYS:-$LEAF_DAYS}"

  VALUES_DNS_SANS_CSV="${KEYCLOAK_CERT_DNS_SANS:-}"
  VALUES_IP_SANS_CSV="${KEYCLOAK_CERT_IP_SANS:-}"
}
...
main() {
  ...
  log "==> Generating (or reusing) CA and issuing one Keycloak server certificate."
  log "    CN: ${CN}"
  ...
  [[ "${#DNS_SANS[@]}" -gt 0 ]] || die "No DNS SANs found. Set DEFAULT_DNS_SANS, values file KEYCLOAK_CERT_DNS_SANS, or pass --dns."

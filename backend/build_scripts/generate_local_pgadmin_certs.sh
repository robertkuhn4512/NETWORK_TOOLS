...
#
# Generates a local CA + a single pgAdmin server certificate/key for the Network Tools project.
#
# NOTES / HOW TO RUN
#   1) Place this script at:
#        $HOME/NETWORK_TOOLS/backend/build_scripts/generate_local_pgadmin_certs.sh
#      Then:
#        chmod +x $HOME/NETWORK_TOOLS/backend/build_scripts/generate_local_pgadmin_certs.sh
#
...
#        $HOME/NETWORK_TOOLS/backend/build_scripts/generate_local_pgadmin_certs.sh \
#          --dns pgadmin \
#          --dns network_tools.local \
#          --dns pgadmin.yourdomain.edu \
#          --ip 127.0.0.1
#
#   6) Values file (optional, auto-loaded if present):
#        Default: $HOME/NETWORK_TOOLS/backend/build_scripts/pgadmin_cert_values.env
#
#        Example contents:
#          PGADMIN_CERT_CN="pgadmin"
#          PGADMIN_CERT_CA_CN="NETWORK_TOOLS Local PgAdmin CA"
#          PGADMIN_CERT_CA_DAYS="3650"
#          PGADMIN_CERT_LEAF_DAYS="825"
#          PGADMIN_CERT_DNS_SANS="pgadmin,network_tools.local,localhost"
#          PGADMIN_CERT_IP_SANS="127.0.0.1"
#          PGADMIN_CERT_OUT_DIR="$PROJECT_ROOT_DEFAULT/backend/app/pgadmin/certs"
#
# OUTPUTS (default):
#   $HOME/NETWORK_TOOLS/backend/app/pgadmin/certs/
#     cert.key   (server key)
...
#
# IMPORTANT (Browsers):
#   If you're using a local CA (ca.crt) your browser/OS will not trust it by default.
#   Import the generated ca.crt into your OS trust store (or your browser) to avoid warnings.
#
...
OUT_DIR_DEFAULT="$PROJECT_ROOT_DEFAULT/backend/app/pgadmin/certs"
VALUES_FILE_DEFAULT="$PROJECT_ROOT_DEFAULT/backend/build_scripts/pgadmin_cert_values.env"

FORCE="0"
SKIP_VALUES_FILE="0"

CN_DEFAULT="pgadmin"
CA_CN_DEFAULT="NETWORK_TOOLS Local PgAdmin CA"
...
# Requirement: include the hostname you browse to in the DNS SANs (e.g., network_tools.local).
DEFAULT_DNS_SANS=(
  "pgadmin"
  "network_tools.local"
  "localhost"
...
Examples:
  $0
  $0 --force
  $0 --dns pgadmin --dns network_tools.local --dns localhost --ip 127.0.0.1
EOF
}
...
# De-dup preserving order; drops blank lines (NF)
dedup_lines() {
...
load_values_file() {
  ...
  OUT_DIR="${PGADMIN_CERT_OUT_DIR:-$OUT_DIR}"
  CN="${PGADMIN_CERT_CN:-$CN}"
  CA_CN="${PGADMIN_CERT_CA_CN:-$CA_CN}"
  CA_DAYS="${PGADMIN_CERT_CA_DAYS:-$CA_DAYS}"
  LEAF_DAYS="${PGADMIN_CERT_LEAF_DAYS:-$LEAF_DAYS}"

  VALUES_DNS_SANS_CSV="${PGADMIN_CERT_DNS_SANS:-}"
  VALUES_IP_SANS_CSV="${PGADMIN_CERT_IP_SANS:-}"
}
...
main() {
  ...
  log "==> Generating (or reusing) CA and issuing one pgAdmin server certificate."
  log "    CN: ${CN}"
...
' "${USER_IP_SANS[@]}"
  ...
  [[ "${#DNS_SANS[@]}" -gt 0 ]] || die "No DNS SANs found. Set DEFAULT_DNS_SANS, values file PGADMIN_CERT_DNS_SANS, or pass --dns."
...
' "${IP_SANS[@]}"
  ...
  # Fullchain (leaf + CA) for pgAdmin convenience
  ...
  log "  ${OUT_DIR}/cert.crt  -> /certs/server.cert"
  log "  ${OUT_DIR}/cert.key  -> /certs/server.key"
  log "  ${OUT_DIR}/ca.crt    -> /certs/ca.crt"
  log ""

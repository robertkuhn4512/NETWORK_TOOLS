...
#
#   3) Run:
#        chmod +x ./backend/app/keycloak/scripts/export_approle_from_vault_keycloak_container.sh
#        ROLE_NAME=keycloak_agent ./backend/app/keycloak/scripts/export_approle_from_vault_keycloak_container.sh
#
...
#   - Intended for the Keycloak Vault Agent container (e.g., vault_agent_keycloak)
#   - If the role does not exist yet, create it first (vault write auth/approle/role/<role> ...)
...
#   OUT_DIR=<repo_root>/container_data/vault/approle/<ROLE_NAME>
...
OUT_DIR="${OUT_DIR:-${PROJECT_ROOT}/container_data/vault/approle/${ROLE_NAME}}"
